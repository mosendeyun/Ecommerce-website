...
var changeNum=(function(){
    return{
        init:function(){
            ...
            this.$inp=document.querySelectorAll('.shop-count')
        ...
        event:function(){
            var _this=this;
            for(var j=0;j<this.$minus.length;j++){
                (function(i){
                    var num=_this.$total[i].innerHTML;
                    num=num.match(/\d+/ig)
                    var val=_this.$inp[i].value;
                    _this.$minus[i].onclick=function(){
                        val=val*1-1
                        if(val<=1){
                            val=1
                        }
                        _this.$inp[i].value=val
                        _this.$total[i].innerHTML=_this.$inp[i].value*num+'元'
                        console.log(val,num)
                    }
                    _this.$plus[i].onclick=function(){
                        val=val*1+1
                        _this.$inp[i].value=val
                        _this.$total[i].innerHTML=_this.$inp[i].value*num+'元'
                        console.log(val,num)
                    ...
                    //     //    console.log(_this.$check[i])              
                    //        if(_this.$check[i].className.indexOf("ifont")>-1){
                    //             _this.$check[i].classList.remove("ifont");
                    //        }else{
                    //            _this.$check[i].classList.add("ifont")
                    //        }
                    //    }
                })(j)
            }
